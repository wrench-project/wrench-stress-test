...
int Simulator::main(int argc, char **argv) {
    ...
    for (unsigned int i=0; i < num_ss; i++) {
        ...
        storage_services.insert(simulation->add<StorageService>(new SimpleStorageService(hostname, {"/"}, {{wrench::SimpleStorageServiceProperty::BUFFER_SIZE, std::to_string(buffer_size)}}, {})));
    }
